const VERSION='onomo-support-it-v8';
const SHELL=['./','./index.html','./assets/pwa/manifest.webmanifest','./assets/js/i18n.js','./assets/js/modern-features.js','./assets/js/runtime-sync.js','./locales/fr.json','./locales/en.json','./locales/ar.json','./assets/pwa/icons/icon-192.png','./assets/pwa/icons/icon-512.png'];
...
self.addEventListener('fetch',event=>{
  ...
  if(request.mode==='navigate'||request.destination==='script'){
    event.respondWith(fetch(request,{cache:'no-store'}).then(response=>{
      ...
      return response;
    }).catch(()=>caches.match(request).then(hit=>{if(hit)return hit;return caches.match(new URL(request.url).pathname).then(fallback=>fallback||caches.match('./index.html'));})));
    return;
  }
