...
(function(){
  ...
  function addVoiceButton(){const r=norm(current()?.role);if(!['demandeur','requester'].includes(r))return;const desc=document.getElementById('ntDesc');if(!desc||document.getElementById('onomoVoiceTicketBtn'))return;const wrap=document.createElement('div');wrap.style.cssText='display:flex;justify-content:flex-end;margin-top:7px';wrap.innerHTML='<button type="button" id="onomoVoiceTicketBtn" class="btn btn-outline btn-sm"><i class="ti ti-microphone"></i> Créer par la voix</button><small style="margin-left:8px;color:var(--tx3);align-self:center">FR / EN / Darija</small>';desc.parentElement?.appendChild(wrap);document.getElementById('onomoVoiceTicketBtn').onclick=startVoice}
