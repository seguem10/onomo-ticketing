...
(function(){
  ...
  function renderRows(rows){
    ...
    return '<div style="overflow:auto"><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">'+t('user','Utilisateur')+'</th><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">'+t('role','Rôle')+'</th><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">'+t('hotel','Hôtel')+'</th><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">'+t('account_security','Sécurité')+'</th><th style="text-align:right;padding:10px;border-bottom:1px solid var(--border)">'+t('actions','Actions')+'</th></tr></thead><tbody>'+filtered.map(u=>{const id=esc(u.id||'');const name=[u.prenom,u.nom].filter(Boolean).join(' ')||u.email||t('user','Utilisateur');const role=roleValues(u)[0]||u.role||'—';const security=u.mfa_enabled?'MFA':t('standard','Standard');let actions='';try{if(typeof openEditUser==='function'||typeof window.openEditUser==='function')actions+='<button class="btn btn-outline btn-sm" onclick="openEditUser(\''+id+'\')"><i class="ti ti-edit"></i>'+t('edit','Modifier')+'</button> ';}catch(_){ }try{if(typeof deleteUser==='function'||typeof window.deleteUser==='function')actions+='<button class="btn btn-danger btn-sm" onclick="deleteUser(\''+id+'\')"><i class="ti ti-trash"></i></button>';}catch(_){ }return '<tr><td style="padding:11px 10px;border-bottom:1px solid var(--border)"><strong>'+esc(name)+'</strong><div style="font-size:11px;color:var(--tx3)">'+esc(u.email||'')+'</div></td><td style="padding:11px 10px;border-bottom:1px solid var(--border)"><span class="role-tag">'+esc(role)+'</span></td><td style="padding:11px 10px;border-bottom:1px solid var(--border)">'+esc(u.hotel||t('all_hotels','Tous les hôtels'))+'</td><td style="padding:11px 10px;border-bottom:1px solid var(--border)">'+security+(u.must_change_password?' · '+t('password_change_required','Mot de passe à changer'):'')+'</td><td style="padding:11px 10px;border-bottom:1px solid var(--border);text-align:right;white-space:nowrap">'+actions+'</td></tr>';}).join('')+'</tbody></table></div>';
  }
  async function render(){
    ...
    main.innerHTML='<div class="card"><div class="card-hdr"><div><div class="card-title">'+t('users','Utilisateurs')+'</div><div style="font-size:11px;color:var(--tx3);margin-top:2px">'+t('user_management_description','Gestion des comptes, rôles et hôtels assignés')+'</div></div><div style="display:flex;gap:8px"><button class="btn btn-outline btn-sm" id="usersRefreshBtn"><i class="ti ti-refresh"></i>'+t('refresh','Actualiser')+'</button><button class="btn btn-gold btn-sm" id="usersCreateBtn"><i class="ti ti-user-plus"></i>'+t('new_user','Nouvel utilisateur')+'</button></div></div><div id="usersPageList" style="padding:0 18px 18px"><div style="padding:30px;text-align:center;color:var(--tx3)">'+t('loading','Chargement...')+'</div></div></div>';
