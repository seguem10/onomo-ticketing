...
(function(){
  ...
  function renderRows(rows){
    ...
    return '<div style="overflow:auto"><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">Utilisateur</th><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">Rôle</th><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">Hôtel</th><th style="text-align:left;padding:10px;border-bottom:1px solid var(--border)">Sécurité</th><th style="text-align:right;padding:10px;border-bottom:1px solid var(--border)">Actions</th></tr></thead><tbody>'+filtered.map(u=>{const id=esc(u.id||'');const name=[u.prenom,u.nom].filter(Boolean).join(' ')||u.email||'Utilisateur';const role=roleValues(u)[0]||u.role||'—';const security=u.mfa_enabled?'MFA':'Standard';return '<tr><td style="padding:11px 10px;border-bottom:1px solid var(--border)"><strong>'+esc(name)+'</strong><div style="font-size:11px;color:var(--tx3)">'+esc(u.email||'')+'</div></td><td style="padding:11px 10px;border-bottom:1px solid var(--border)"><span class="role-tag">'+esc(role)+'</span></td><td style="padding:11px 10px;border-bottom:1px solid var(--border)">'+esc(u.hotel||'Tous les hôtels')+'</td><td style="padding:11px 10px;border-bottom:1px solid var(--border)">'+security+(u.must_change_password?' · Mot de passe à changer':'')+'</td><td style="padding:11px 10px;border-bottom:1px solid var(--border);text-align:right;white-space:nowrap">'+(typeof window.openEditUser==='function'?'<button class="btn btn-outline btn-sm" onclick="openEditUser(\''+id+'\')"><i class="ti ti-edit"></i>Modifier</button> ':'')+(typeof window.deleteUser==='function'?'<button class="btn btn-danger btn-sm" onclick="deleteUser(\''+id+'\')"><i class="ti ti-trash"></i></button>':'')+'</td></tr>';}).join('')+'</tbody></table></div>';
  }
  ...
  function bind(){
    ...
    if(item.dataset.usersFixBound==='1')return;
  ...
  function forceSwitch(){
    ...
    const wrapped=function(view,el){if(view==='users'){render();return;}return original.apply(this,arguments);};
