...
(function(){
  ...
  function hotelLabel(u){
    ...
    return t('not_defined','Non défini');
  }

  function renderProfile(u){
    const main=document.getElementById('mainContent');
    if(!main)return;
    const name=`${u?.prenom||''} ${u?.nom||''}`.trim()||u?.email||t('user','Utilisateur');
    const email=u?.email||'';
    ...
    main.innerHTML=`
      <div style="max-width:700px">
        <div class="page-head" style="margin-bottom:18px">
          <div><h2>${t('profile','Mon profil')}</h2><p>${t('profile_description','Informations de votre compte et changement de mot de passe.')}</p></div>
        </div>
        <div class="card" style="margin-bottom:16px;padding:18px">
          <div style="font-weight:600;font-size:15px;margin-bottom:16px;color:var(--tx)">${t('account_info','Informations du compte')}</div>
          <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px">
            <div><div style="font-size:11px;color:var(--tx2);margin-bottom:4px">${t('name','Nom')}</div><div style="color:var(--tx);font-weight:500">${esc(name)}</div></div>
            <div><div style="font-size:11px;color:var(--tx2);margin-bottom:4px">${t('email','Email')}</div><div style="color:var(--tx);font-weight:500">${esc(email)}</div></div>
            <div><div style="font-size:11px;color:var(--tx2);margin-bottom:4px">${t('role','Rôle')}</div><div style="color:var(--tx);font-weight:500">${esc(role)}</div></div>
            <div><div style="font-size:11px;color:var(--tx2);margin-bottom:4px">${t('hotel','Hôtel')}</div><div style="color:var(--tx);font-weight:500">${esc(hotel)}</div></div>
          </div>
        </div>
        <div class="card" style="padding:18px">
          <div style="font-weight:600;font-size:15px;margin-bottom:16px;color:var(--tx)">${t('password','Mot de passe')}</div>
          <div style="margin-bottom:14px"><label style="display:block;font-size:11px;color:var(--tx2);margin-bottom:5px">${t('current_password','Mot de passe actuel')}</label><input id="pOld" type="password" autocomplete="current-password" style="width:100%;padding:11px 12px;border:1px solid var(--border);border-radius:var(--r);background:var(--surface);color:var(--tx);outline:none"></div>
          <div style="margin-bottom:14px"><label style="display:block;font-size:11px;color:var(--tx2);margin-bottom:5px">${t('password_new','Nouveau mot de passe')}</label><input id="pNew" type="password" autocomplete="new-password" style="width:100%;padding:11px 12px;border:1px solid var(--border);border-radius:var(--r);background:var(--surface);color:var(--tx);outline:none"></div>
          <div style="margin-bottom:16px"><label style="display:block;font-size:11px;color:var(--tx2);margin-bottom:5px">${t('confirm_new_password','Confirmer le nouveau mot de passe')}</label><input id="pConfirm" type="password" autocomplete="new-password" style="width:100%;padding:11px 12px;border:1px solid var(--border);border-radius:var(--r);background:var(--surface);color:var(--tx);outline:none"></div>
          <button class="btn-primary" type="button" onclick="if(typeof window.changeMyPassword==='function'){window.changeMyPassword()}else if(typeof window.showToast==='function'){window.showToast('${t('feature_unavailable','Fonction indisponible.')}','err')}">${t('save_new_password','Enregistrer le nouveau mot de passe')}</button>
        </div>
      </div>`;
  }
  ...
  async function openProfile(event){
    ...
    if(title)title.textContent=t('profile','Mon profil');
    try{
      const user=await resolveUser();
      if(!user)throw new Error(t('user_not_found','Utilisateur introuvable'));
      renderProfile(user);
    }catch(error){
      console.error('[ONOMO] profil robuste impossible',error);
      const main=document.getElementById('mainContent');
      if(main)main.innerHTML='<div class="card" style="padding:20px"><h3 style="margin-bottom:8px">'+t('profile','Mon profil')+'</h3><p style="color:var(--tx2)">'+t('profile_load_failed','Impossible de charger les informations du profil. Veuillez actualiser la page.')+'</p></div>';
    }
  }
