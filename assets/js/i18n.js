...
(function(){
  ...
  function makeSelector(id){
    ...
    select.value=active;select.addEventListener('change',event=>setLanguage(event.target.value));
    return select;
  }
  function selector(){
    /* Homepage: language selector at the top-right. */
    if(!document.getElementById('languageSelector-login')){
    ...
    if(!document.getElementById('languageSelector-app')){
      ...
      if(topbar&&search){
        ...
        appSelect.style.setProperty('right','auto','important');
  ...
  document.addEventListener('DOMContentLoaded',()=>{active=normalize(localStorage.getItem('onomo_language')||detect());selector();setLanguage(active,false);new MutationObserver(mutations=>{if(mutations.some(m=>m.addedNodes.length)){selector();translateLegacy();}}).observe(document.body,{childList:true,subtree:true});});
})();
