...
(function(){
  ...
  const getSelectedAssignee=()=>{
    const selects=[...document.querySelectorAll('select')].filter(sel=>{
      ...
      return /(assign|assignee|assigned|responsable|technicien|agent|assigne)/.test(meta);
    });
    for(const sel of selects){
      const value=sel.value||sel.dataset.onomoSelected||'';
      const text=sel.selectedOptions?.[0]?.textContent||'';
      const u=resolveTicketUser(value)||resolveTicketUser(text)||resolveTicketUser(sel.dataset.onomoSelected||'');
      if(u)return u;
    }
    return null;
  };
  const normalizeAssignment=(updates,id)=>{const out={...(updates||{})};let raw=out.assigned_to??out.assignee_id??out.assigne_a??'';if(!raw){const t=findTicket(id);raw=t?.assigned_to||t?.assigne_a||'';if(!raw&&N(out.statut)==='ferme'){const d=defaultIT(t?.hotel);raw=d?.auth_user_id||d?.id||d?.email||''}}const u=resolveTicketUser(raw);if(u){out.assigned_to=u.auth_user_id||u.id||null;out.assigne_a=u.email||(`${u.prenom||''} ${u.nom||''}`).trim()}else if(raw){out.assigne_a=String(raw).trim()}delete out.assignee_id;return out};
  function assignmentSelect(sel){
    if(!sel)return;
    const users=ticketUsers();if(!users.length)return;
    const old=sel.value,text=sel.selectedOptions?.[0]?.textContent||'',previous=resolveTicketUser(old)||resolveTicketUser(text)||resolveTicketUser(sel.dataset.onomoSelected||'');
    ...
    if(!sel.__onomoChangeBound){
      sel.addEventListener('change',()=>{sel.dataset.onomoSelected=sel.value||'';const u=resolveTicketUser(sel.value)||resolveTicketUser(sel.selectedOptions?.[0]?.textContent||'');if(u)sel.dataset.onomoSelected=String(u.auth_user_id||u.id||u.email)});
  ...
  function installAssignment(){
    ...
    const c=window.createTicket;if(typeof c==='function'&&!c.__onomoCorrectionWrapped){const w=async function(data){
      const p={...(data||{})};
      const selected=getSelectedAssignee();
      if(selected){
        p.assigned_to=selected.auth_user_id||selected.id||selected.email||null;
        p.assigne_a=selected.email||(`${selected.prenom||''} ${selected.nom||''}`).trim();
      }else if(!p.assigned_to&&!p.assignee_id&&!p.assigne_a){
        const d=defaultIT(p.hotel);if(d){p.assigned_to=d.auth_user_id||d.id||d.email;p.assigne_a=d.email||(`${d.prenom||''} ${d.nom||''}`).trim()}
