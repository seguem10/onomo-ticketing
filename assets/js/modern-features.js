...
(function(){
  ...
  window.addEventListener('DOMContentLoaded',()=>{ensureRequesterRoleOption();setInterval(()=>{const started=Number(sessionStorage.getItem('onomo_session_started')||0);if(currentUser&&started&&Date.now()-started>8*60*60*1000){showToast('Votre session a expiré.','err');doLogout();}},60000);});
