...
(function(){
  ...
  function getAuthClient(){
    ...
    if(!window.supabase||!window.settings?.sbUrl||!window.settings?.sbKey)return null;
    try{
      authClient=window.supabase.createClient(window.settings.sbUrl,window.settings.sbKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});
      return authClient;
  ...
  /* The legacy REST helper was sending the publishable key as Bearer token.
     That authenticates the API key but not the logged-in user, so RLS saw an
     anonymous request. Replace it with an authenticated REST helper. */
  function installAuthenticatedSbFetch(){
    ...
    window.sbFetch=async function(path,opts={}){
      if(!window.settings?.sbUrl||!window.settings?.sbKey)throw new Error('Supabase non configuré');
      const session=await getAuthSession();
      const headers={
        'Content-Type':'application/json',
        'Prefer':opts.prefer||'return=representation',
        'apikey':window.settings.sbKey,
        'Authorization':`Bearer ${session?.access_token||window.settings.sbKey}`
      };
      const response=await fetch(`${window.settings.sbUrl}/rest/v1/${path}`,{...opts,headers});
      if(!response.ok){
  ...
  async function syncTickets(){
    ...
    }catch(error){console.warn('Ticket sync unavailable',error);}
  ...
  function startRealtime(){
    ...
    try{
      realtimeClient=getAuthClient()||window.supabase.createClient(window.settings.sbUrl,window.settings.sbKey,{auth:{persistSession:true,autoRefreshToken:true}});
  ...
  window.doLogin=async function(){
    ...
    if(client&&email&&pwd){
      ...
      }catch(error){console.warn('Supabase Auth login échoué:',error.message||error);}
    }
    installAuthenticatedSbFetch();
  ...
  window.initSession=function(){previousInit();writeSession();touch();startSync();};
  ...
  window.createTicket=async function(data){
    ...
    else console.warn('Création ticket sans session Supabase Auth. La RLS peut refuser l\'écriture.');
  ...
  window.addComment=async function(message){
    installAuthenticatedSbFetch();
    if(currentTicket?.statut==='fermé'&&['admin','it_regional','it_hotel'].includes(currentUser?.role))await updateTicket(currentTicket.id,{statut:'nouveau'});
