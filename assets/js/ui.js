(function(){
  ...
  window.onomoUI={notify(message){const toast=document.getElementById('toast');if(!toast)return;toast.textContent=message;window.OnomoI18n?.setLanguage(window.OnomoI18n.language,false).catch?.(()=>{});toast.classList.add('show');window.setTimeout(()=>toast.classList.remove('show'),3200)}};
})();
