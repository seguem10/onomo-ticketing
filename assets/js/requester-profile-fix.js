/* ONOMO Support IT - Demandeur: liste IT + secours Mon profil */
(function(){
  'use strict';
  const roleNorm=r=>({admin:'admin',administrateur:'admin','it régional':'it_regional',it_regional:'it_regional','it hotel':'it_hotel',it_hotel:'it_hotel',demandeur:'demandeur',requester:'demandeur'}[String(r||'').toLowerCase().trim()]||String(r||'').toLowerCase().trim());
  const hotelsOf=u=>{
  ...
  function profileFallback(){
    ...
    if(!main)return;
    main.innerHTML=`<div style="max-width:480px"><div class="page-head"><div><h2>Mon profil</h2><p>Informations de votre compte</p></div></div><div class="card" style="padding:20px;margin-top:14px"><div class="settings-title">Informations du compte</div><div style="display:grid;gap:12px;margin-top:14px"><div><div class="form-lbl" style="color:var(--tx2)">Prénom</div><div class="form-ctrl" style="color:var(--tx);background:var(--surface2)">${u.prenom||'—'}</div></div><div><div class="form-lbl" style="color:var(--tx2)">Nom</div><div class="form-ctrl" style="color:var(--tx);background:var(--surface2)">${u.nom||'—'}</div></div><div><div class="form-lbl" style="color:var(--tx2)">Email</div><div class="form-ctrl" style="color:var(--tx);background:var(--surface2)">${u.email||'—'}</div></div><div><div class="form-lbl" style="color:var(--tx2)">Rôle</div><div class="form-ctrl" style="color:var(--tx);background:var(--surface2)">${roleNorm(u.role)==='demandeur'?'Demandeur':u.role||'—'}</div></div><div><div class="form-lbl" style="color:var(--tx2)">Hôtel</div><div class="form-ctrl" style="color:var(--tx);background:var(--surface2)">${hotel}</div></div></div></div></div>`;
  }
  function installProfileFallback(){
    ...
    const wrapped=function(view,el){
      if(view==='profile'&&roleNorm(window.currentUser?.role)==='demandeur'){
        try{return original.apply(this,arguments);}catch(e){console.error('ONOMO profil demandeur:',e);profileFallback();return;}
      }
  ...
  function boot(){
    installProfileFallback();
    setTimeout(fillRequesterAgent,300);
    setTimeout(fillRequesterAgent,1200);
    setTimeout(fillRequesterAgent,2500);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  let tries=0;const timer=setInterval(()=>{if(installProfileFallback()||++tries>30)clearInterval(timer);},250);
})();
